...
module.exports = class OverlayManager {
  ...
  hoverTargetKind(editorView, event) {
    const target = lumine.dom.isElement(event.target) ? event.target : null;
  ...
  mountOverlay(editor, range, position, element, { type, highlight, markerInvalidate = "never" }) {
    ...
    element.addEventListener("mouseup", () => {
      if (element.contains(activeElementFor(element)) && !this.overlaySelection()) {
        view.focus();
      }
    });
    ...
    element.addEventListener(
      "wheel",
      (event) => {
        let node = lumine.dom.isElement(event.target) ? event.target : null;
  ...
  unmountOverlay() {
    ...
    if (this.overlayElement?.contains(activeElementFor(this.overlayElement))) {
...
module.exports.activeElementFor = activeElementFor;
