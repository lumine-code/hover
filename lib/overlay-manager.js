...
module.exports = class OverlayManager {
  ...
  // Runs on every pointer move, ahead of the delay that requests a tooltip. A
  // tooltip belongs to the range it was asked about, and the pointer leaving
  // that range retires it after `hideDelay` whether or not the pointer ever
  // comes to rest — waiting for it to stop is what left the tooltip standing
  // over text it no longer describes.
  trackPointer(event) {
    if (this.currentOverlayType !== "hover") return;
    if (this.isPointerWithinAnchor(event)) this.hideTimer.unschedule();
    else this.hideTimer.scheduleUnlessPending();
  }

  isPointerWithinAnchor(event) {
    const rect = this.currentAnchorRect;
    if (!rect || !this.editorView) return false;
    const { left, top } = this.editorView.getComponent().pixelPositionForMouseEvent(event);
