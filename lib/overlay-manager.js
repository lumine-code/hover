...
module.exports = class OverlayManager {
  ...
  mountOverlay(editor, range, position, element, { type, highlight, markerInvalidate = "never" }) {
    ...
    element.addEventListener("mouseup", () => {
      if (element.contains(lumine.dom.activeElementFor(element)) && !this.overlaySelection()) {
        view.focus();
      }
    });
  ...
  unmountOverlay() {
    ...
    if (this.overlayElement?.contains(lumine.dom.activeElementFor(this.overlayElement))) {
