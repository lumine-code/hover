...
module.exports = class OverlayManager {
  ...
  async onMouseRemain(event) {
    ...
    if (event.target?.closest?.(".gutter-container")) {
      if (!this.currentMarkerRange?.containsPoint(point)) {
        await this.showHoverOverlay(this.editor, point, { gutter: true });
      }
      return;
    }
    ...
    if (Math.abs(mouse.left - screen.left) >= this.editor.getDefaultCharWidth()) return;
  ...
  // first non-empty answer. Over the gutter the question is about the whole
  // row rather than a position on it, and only providers with something to
  // say about a row answer it at all.
  async showHoverOverlay(editor, position, { gutter = false } = {}) {
    try {
      let result = null;
      ...
      if (!result?.contents?.value && !result?.contents?.element) {
      ...
      // A gutter answer stands for the whole row unless it says otherwise, so
      // that the tooltip survives the pointer travelling along the line it
      // describes — the row is what was asked about.
      const reported =
        result.range ?? (gutter ? editor.getBuffer().rangeForRow(position.row) : null);
      const range = reported ? Range.fromObject(reported) : null;
  ...
  unmountOverlay() {
    ...
    if (this.overlayElement?.contains(document.activeElement)) this.editorView?.focus();
    this.hideTimer?.unschedule();
    this.currentMarkerRange = null;
    this.currentOverlayType = null;
    this.currentAnchorRect = null;
