...
async function renderMarkdownFragment(markdown, document) {
  ...
  if (fragment.ownerDocument !== document) document.adoptNode(fragment);
...
function wrapInPanel(view, document) {
...
async function renderSection(contents, document) {
  ...
  const fragment = await renderMarkdownFragment(contents.value, document);
...
async function renderHoverContent(contentsList, document = globalThis.document) {
  const view = createView(document);
  for (const contents of [contentsList].flat()) {
    const section = await renderSection(contents, document);
    if (section) view.appendChild(section);
  }
  if (!view.firstChild) return null;
  return wrapInPanel(view, document);
}
...
async function renderSignatureContent(
  signatureHelp,
  { includeSignatureDocumentation = false, document = globalThis.document } = {},
) {
  ...
  if (docs) {
    const fragment = await renderMarkdownFragment(docs, document);
